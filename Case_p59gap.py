...
# c --- natural B for poison sleeves$
M20 = openmc.Material(name='natB')
M20.set_density('atom/b-cm', 0.0250690)
M20.temperature = 300.0
#M20.id = 20
M20.add_nuclide('B10'  , 0.0049887, 'ao')
M20.add_nuclide('B11'  , 0.0200803, 'ao')

# c ----cadmium -----------$
M30 = openmc.Material(name='cadmium')
M30.set_density('atom/b-cm', 0.0463402)
M30.temperature = 300.0
M30.add_element('Cd'   , 0.0463402, 'ao')

# c ---- Steel 304---------$
M35 = openmc.Material(name='Steel 304')
M35.set_density('atom/b-cm', 0.0862393)
M35.temperature = 300.0
M35.add_nuclide('Fe54', 0.0035020, 'ao')
M35.add_nuclide('Fe56', 0.0544408, 'ao')
M35.add_nuclide('Fe57', 0.0012465, 'ao')
M35.add_nuclide('Fe58', 0.0001662, 'ao')
M35.add_nuclide('Ni58', 0.0052698, 'ao')
M35.add_nuclide('Ni60', 0.0020147, 'ao')
M35.add_nuclide('Ni61', 0.0000872, 'ao')
M35.add_nuclide('Ni62', 0.0002771, 'ao')
M35.add_nuclide('Ni64', 0.0000702, 'ao')
M35.add_nuclide('Cr50', 0.0007573, 'ao')
M35.add_nuclide('Cr52', 0.0146033, 'ao')
M35.add_nuclide('Cr53', 0.0016557, 'ao')
M35.add_nuclide('Cr54', 0.0004122, 'ao')
M35.add_nuclide('Mn55', 0.0017363, 'ao')
...
###############################################################################
materials = openmc.Materials([M5, M6, M7, M8, M9, M10, M11, M12,
  M20, M30, M35,
...
s21 = openmc.ZPlane(   surface_id=21, z0=15.99565   )
...
#s36 = openmc.ZCylinder(surface_id=36, r=13.25499 )
#s37 = openmc.ZPlane(   surface_id=37, z0=10.50925 )
...
#c ----- radial sleeve surfaces -----------------$
s500 = openmc.ZCylinder(surface_id=500, r= 11.50874)
s501 = openmc.ZCylinder(surface_id=501, r= 11.58748)
s502 = openmc.ZCylinder(surface_id=502, r= 19.05   )
s503 = openmc.ZCylinder(surface_id=503, r= 19.12874)
s504 = openmc.ZCylinder(surface_id=504, r= 19.28749)
s505 = openmc.ZCylinder(surface_id=505, r= 19.36623)
s506 = openmc.ZCylinder(surface_id=506, r= 21.74875)
s507 = openmc.ZCylinder(surface_id=507, r= 21.82749)
s508 = openmc.ZCylinder(surface_id=508, r= 21.98624)
s509 = openmc.ZCylinder(surface_id=509, r= 22.62124)
# c ---- axial sleeve surfaces ------------------$
s600 = openmc.ZPlane(   surface_id=600, z0= 15.7607  )
s601 = openmc.ZPlane(   surface_id=601, z0= 15.68196 )
s602 = openmc.ZPlane(   surface_id=602, z0= 6.30047  )
s603 = openmc.ZPlane(   surface_id=603, z0= 5.98551  )
s604 = openmc.ZPlane(   surface_id=604, z0= -15.7607 )
s605 = openmc.ZPlane(   surface_id=605, z0= -15.68196)
...
# CELLS define
# cell: 300
C300 = openmc.Cell(cell_id=300, fill=M7, region=-s6 & -s8 & +s9)
...
# add cells
C301 = openmc.Cell(cell_id=301,fill=M6,region=-s6 & +s8  & -s10)
C302 = openmc.Cell(cell_id=302,fill=M6,region=-s6 & -s9  & +s11)
C303 = openmc.Cell(cell_id=303,fill=M7,region=-s6 & +s10 & -s12)
C304 = openmc.Cell(cell_id=304,fill=M7,region=-s6 & -s11 & +s13)
C305 = openmc.Cell(cell_id=305,fill=M9,region=(-s7 & -s13 & +s15)|(-s7 & +s6 & -s14 & +s13)|(-s16 & +s6 & -s12 & +s14))
C306 = openmc.Cell(cell_id=306, fill=M6 , region=(-s6 &-s17 &+s12)|(-s16 &+s6 &-s18 &+s12) )
un.add_cells([C301,C302,C303,C304,C305,C306])

# c --------sleeve assembly-----------------------$
C311 = openmc.Cell(cell_id=311, fill=M7 , region= (+s7  & -s500& +s19  &-s600)     )
C312 = openmc.Cell(cell_id=312, fill=M7 , region= (+s7  & -s500& -s20  &+s604)     )
C313 = openmc.Cell(cell_id=313, fill=M35, region= (+s500& -s501& -s601 &+s602) )
C314 = openmc.Cell(cell_id=314, fill=M35, region= (+s500& -s501& +s605& -s603) )
C315 = openmc.Cell(cell_id=315,           region= (+s501& -s502& +s602& -s601) )
C316 = openmc.Cell(cell_id=316,           region= (+s501& -s502& +s605& -s603) )
C317 = openmc.Cell(cell_id=317, fill=M35, region= (+s502& -s503& +s602& -s601) )
C318 = openmc.Cell(cell_id=318, fill=M35, region= (+s502& -s503& +s605& -s603) )
C319 = openmc.Cell(cell_id=319, fill=M35, region= (+s500& -s503& +s601& -s600) )
C320 = openmc.Cell(cell_id=320, fill=M35, region= (+s500& -s503& +s19 & -s602)  )
C321 = openmc.Cell(cell_id=321, fill=M35, region= (+s500& -s503& -s605 &+s604) )
C322 = openmc.Cell(cell_id=322, fill=M35, region= (+s500& -s503& -s20  &+s603)  )
C331 = openmc.Cell(cell_id=331, fill=M7 , region= (+s503& -s504& +s19  &-s600)   )
C332 = openmc.Cell(cell_id=332, fill=M7 , region= (+s503& -s504& -s20  &+s604)   )
C333 = openmc.Cell(cell_id=333, fill=M35, region= (+s504& -s505& +s602&-s601) )
C334 = openmc.Cell(cell_id=334, fill=M35, region= (+s504& -s505& +s605&-s603) )
C335 = openmc.Cell(cell_id=335, fill=M20, region= (+s505& -s506& +s602&-s601) )
C336 = openmc.Cell(cell_id=336, fill=M20, region= (+s505& -s506& +s605&-s603) )
C337 = openmc.Cell(cell_id=337, fill=M35, region= (+s506& -s507& +s602&-s601) )
C338 = openmc.Cell(cell_id=338, fill=M35, region= (+s506& -s507& +s605&-s603) )
C339 = openmc.Cell(cell_id=339, fill=M35, region= (+s504& -s507& +s601&-s600) )
C340 = openmc.Cell(cell_id=340, fill=M35, region= (+s504& -s507& +s19 &-s602)  )
C341 = openmc.Cell(cell_id=341, fill=M35, region= (+s504& -s507& -s605 &+s604) )
C342 = openmc.Cell(cell_id=342, fill=M35, region= (+s504& -s507& -s20  &+s603)  )
C351 = openmc.Cell(cell_id=351, fill=M7 , region= (+s507& -s508& +s19  &-s600)   )
C352 = openmc.Cell(cell_id=352, fill=M7 , region= (+s507& -s508& -s20 &+s604)   )
C353 = openmc.Cell(cell_id=353, fill=M35, region= (+s19 & -s600& +s508 &-s509)  )
C354 = openmc.Cell(cell_id=354, fill=M35, region= (+s604& -s20 & +s508 &-s509)  )
C355 = openmc.Cell(cell_id=355, fill=M7, region= (+s7  & -s509& +s600 &-s8)     )
C356 = openmc.Cell(cell_id=356, fill=M7, region= (+s7  & -s509& +s9   &-s604)     )

un.add_cells([C311, C312, C313, C314, C315, C316, C317, C318, C319, C320])
un.add_cells([C321, C322, C331, C332, C333, C334, C335, C336, C337, C338])
un.add_cells([C339, C340, C341, C342, C351, C352, C353, C354, C355, C356])
# c ----------- water tanks -----------------------------------$
C407 = openmc.Cell(cell_id=407, fill=M7, region= (+s29 &-s15 &-s27)|(+s9 &-s20 &-s27 &+s509)|(-s27 &+s7 &+s15 &-s9) )
C408 = openmc.Cell(cell_id=408, fill=M7, region= (+s20 &-s31 &+s24 &-s27))
C409 = openmc.Cell(cell_id=409, fill=M9, region= (+s26 &-s25 &-s28)|(+s27 &-s28 &-s31 &+s25) )
C410 = openmc.Cell(cell_id=410, region= -s27 &+s25 &-s30 )
C411 = openmc.Cell(cell_id=411, fill=M9, region= -s29 &+s30 &-s27 )
C412 = openmc.Cell(cell_id=412, region= +s31 &-s22 &+s24 &-s28 )
C413 = openmc.Cell(cell_id=413, fill=M9, region= (-s19 &+s20 &+s7 &-s24)|(-s24 &+s23 &+s19 &-s22) )
C414 = openmc.Cell(cell_id=414, fill=M7, region= (+s19 &-s8 &+s509 &-s23)|(+s8 &-s21 &-s23 &+s7)) 
C415 = openmc.Cell(cell_id=415, region= (+s21 &-s14 &+s7 &-s23)|(+s14 &-s18 &+s16 &-s23)|(+s17 &-s18 &-s6)|(+s18 &-s22 &-s23) )

un.add_cells([C407, C408, C409, C410, C411, C412, C413, C414, C415])

C416 = openmc.Cell(cell_id=416, region=(+s22 &-s999)|(-s22 &+s31 &+s28 &-s999)|(+s28 &-s999 &+s26 &-s31)|(-s26 &-s999)) 
un.add_cell(C416)
