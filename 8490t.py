...
# 3.24 kg borate in 39 gal of water
M2 = openmc.Material(name='boron water')
M2.temperature = 300.0
#M2.id = 2
M2.set_density('atom/b-cm', 0.1001638)
M2.add_nuclide('B10' ,  0.0000475, 'ao')
M2.add_nuclide('B11' ,  0.0001914, 'ao')
M2.add_nuclide('H1'  ,  0.0663936, 'ao')
M2.add_nuclide('N14' ,  0.0000478, 'ao')
M2.add_nuclide('O16' ,  0.0334835, 'ao')
...
M11.set_density('atom/b-cm', 0.1216161)
...
###############################################################################
materials = openmc.Materials([M1, M2, M5, M6, M7, M8, M9, M11])
...
#s81  = openmc.ZCylinder(surface_id=81,  r=13.92555  )
...
# CELLS define
# cell: 100
C100 = openmc.Cell(cell_id=100,fill=M2,region=-s6 & -s8 & +s9)
...
un.add_cells([C29, C30, C31, C32, C33, C34, C35])
...
C103 = openmc.Cell(cell_id=103,fill=M2,region=-s6 & +s10 & -s12)
C104 = openmc.Cell(cell_id=104,fill=M2,region=-s6 & -s11 & +s13)
...
#C109 = openmc.Cell(cell_id=109,fill=M3, region=(+s7 &-s86 &+s19 &-s83)|(+s7 &-s86 &+s82 &-s14)|(+s85 &-s86 &+s83 &-s82))
C107 = openmc.Cell(cell_id=107,fill=M2, region=(-s23 &+s7 &-s14 &+s19)|(-s23 &+s16 &-s18 &+s14)|(-s6 &-s18 &+s17)|(-s23 &-s21 &+s18))
...
#C113 = openmc.Cell(cell_id=113,fill=M12, region=(-s81 &+s7 &-s75 &+s78))

C110 = openmc.Cell(cell_id=110,fill=M2,region=(-s27 &+s7 &-s20 &+s15)|(-s27 &-s15 &+s25))
